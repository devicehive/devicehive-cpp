    ...
    public gateway::SerialModule
{
    ...
    typedef ApplicationREST This; ///< @brief The this type alias.
protected:
...
public:
    ...
    typedef boost::shared_ptr<ApplicationREST> SharedPtr;
...
/**
Creates the ApplicationREST instance and calls its ApplicationREST::run() method.
...
inline void main(int argc, const char* argv[], bool useNewWS = true)
{
    ...
    if (useNewWS)
        ...
        ApplicationREST::create(argc, argv)->run();
}
...
simple_gw::ApplicationREST::create() factory method. Application does all
its useful work in simple_gw::ApplicationREST::run() method.
...
the server using simple_gw::ApplicationREST::asyncRegisterDevice() method.

If registration is successful we start listening for commands from the
DeviceHive server using simple_gw::ApplicationREST::asyncPollCommands() method.
